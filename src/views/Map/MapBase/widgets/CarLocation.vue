<template>
  <BaseCollapse title="工程车辆定位" width="300px" :loading="loading">
    <BoardPanel>
      <a-input
        v-model="query"
        size="small"
...
const { dataSource, query, loading, run } =
  useLocationTable<LocationCarListType>(locationCarList);
const onRowClick = (record: LocationCarListType) =>
  mapInstance?.flyto(record.coordinates, 16);
